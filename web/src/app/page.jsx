...
export default function Home() {
    ...
    const vote = (win) => {
        if (win) {
            fetch("http://localhost:8000/vote/?" + new URLSearchParams({"thingid": currentThing["uuid"]}))
        } else {
            fetch("http://localhost:8000/appeared/?" + new URLSearchParams({"thingid": currentThing["uuid"]}))
        }
        setCurrent(things[1])
